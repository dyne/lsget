...
func TestCleanJoinRealFromVirtual(t *testing.T) {
	s := newTestServer(t)
	// cleanVirtual
	if cleanVirtual("") != "/" { t.Fatal("cleanVirtual empty -> /") }
	if cleanVirtual("foo/..//bar") != "/bar" { t.Fatal("cleanVirtual normalize") }
	// joinVirtual
	if joinVirtual("/a", "b") != "/a/b" { t.Fatal("join relative") }
	if joinVirtual("/a", "/x") != "/x" { t.Fatal("join absolute wins") }
	// realFromVirtual root
	r, err := s.realFromVirtual("/")
	if err != nil || r != s.rootAbs { t.Fatalf("realFromVirtual root: %v %q", err, r) }
...
func TestParseArgs(t *testing.T) {
	...
	if len(args) != len(exp) { t.Fatalf("args len: %v", args) }
	for i := range exp {
		if args[i] != exp[i] { t.Fatalf("arg %d = %q", i, args[i]) }
	}
}

func TestLooksText(t *testing.T) {
	if looksText([]byte{0x00, 'a'}) { t.Fatal("NUL should be binary") }
	if !looksText([]byte("hello")) { t.Fatal("simple ascii is text") }
	if !looksText([]byte{}) { t.Fatal("empty is text") }
}
...
func TestGetFileColorAndColorizeName(t *testing.T) {
	...
	if err := os.Mkdir(dir, 0o755); err != nil { t.Fatal(err) }
	info, _ := os.Stat(dir)
	if c := getFileColor(info, "dir"); c != colorBlue+colorBold { t.Fatalf("dir color: %q", c) }
	// executable
	exe := filepath.Join(root, "exe.sh")
	if err := os.WriteFile(exe, []byte("#!/bin/sh\n"), 0o755); err != nil { t.Fatal(err) }
	info, _ = os.Stat(exe)
	if c := getFileColor(info, "exe.sh"); c != colorGreen { t.Fatalf("exe color: %q", c) }
	// symlink
	ln := filepath.Join(root, "link")
	if err := os.Symlink(exe, ln); err != nil { t.Fatal(err) }
	linfo, _ := os.Lstat(ln)
	if c := getFileColor(linfo, "link"); c != colorGreen { t.Fatalf("symlink color: %q", c) }
	// extension mapping
	mp := filepath.Join(root, "f.zip")
	if err := os.WriteFile(mp, []byte("z"), 0o644); err != nil { t.Fatal(err) }
	minfo, _ := os.Stat(mp)
	if c := getFileColor(minfo, "f.zip"); c != colorRed { t.Fatalf("zip color: %q", c) }
	// colorize wraps
...
func TestFormatLong(t *testing.T) {
	...
	data := []byte("abc")
	if err := os.WriteFile(f, data, 0o644); err != nil { t.Fatal(err) }
	info, _ := os.Stat(f)
...
func TestURLEscapeHelpers(t *testing.T) {
	...
	if !strings.Contains(esc, "%20") || !strings.HasPrefix(esc, "/") { t.Fatalf("urlEscapeVirtual: %q", esc) }
	if urlQueryEscape("a b") != "a%20b" { t.Fatal("urlQueryEscape space") }
}

func TestNewSIDAndGetSession(t *testing.T) {
	id := newSID()
	if len(id) != 32 { t.Fatalf("sid length: %d", len(id)) }
	if _, err := hex.DecodeString(id); err != nil { t.Fatalf("sid hex: %v", err) }

	s := newTestServer(t)
	r := httptest.NewRequest("GET", "/", nil)
	w := httptest.NewRecorder()
	sess := s.getSession(w, r)
	if sess == nil || sess.cwd != "/" { t.Fatal("new session cwd") }
	ck := w.Result().Cookies()
	if len(ck) == 0 || ck[0].Name != "sid" { t.Fatal("sid cookie not set") }
	// Second time with cookie should return existing
	r2 := httptest.NewRequest("GET", "/", nil)
	r2.AddCookie(ck[0])
	w2 := httptest.NewRecorder()
	sess2 := s.getSession(w2, r2)
	if sess2 != sess { t.Fatal("session not reused") }
}
...
func TestProcessHTMLTemplate(t *testing.T) {
	...
	so := string(out)
	if strings.Contains(so, "{{HELP_MESSAGE}}") { t.Fatal("HELP_MESSAGE not replaced") }
	if !strings.Contains(so, "/foo/bar") { t.Fatal("INITIAL_PATH not injected") }
}
...
func TestHandleIndexServesAndFile(t *testing.T) {
	...
	r := httptest.NewRequest("GET", "/", nil)
	s.handleIndex(w, r)
	if w.Result().StatusCode != 200 { t.Fatalf("index status: %d", w.Result().StatusCode) }
	// request file
	w2 := httptest.NewRecorder()
...
func TestHandleStaticFile(t *testing.T) {
	...
	if w.Code != 200 { t.Fatalf("static status: %d", w.Code) }
}
...
func TestFormatLongTimeStability(t *testing.T) {
	...
	if err := os.WriteFile(f, bytes.Repeat([]byte{'A'}, 10), 0o644); err != nil { t.Fatal(err) }
	info, _ := os.Stat(f)
